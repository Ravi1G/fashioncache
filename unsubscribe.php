...
\*******************************************************************/
	...
	if (isset($_GET['key']) && is_string($_GET['key']) && preg_match('/^[a-z\d]{32}$/i', $_GET['key']))
	{
		...
		$check_result = smart_mysql_query("SELECT newsletter FROM cashbackenigne_users WHERE unsubscribe_key='$unsubscribe_key' LIMIT 1");
        if (mysql_num_rows($check_result) > 0)
		{
			...
			{
				smart_mysql_query("UPDATE cashbackenigne_users SET newsletter='0' WHERE unsubscribe_key='$unsubscribe_key' LIMIT 1");
				header ("Location: unsubscribe.php?msg=1");
				exit();
			}
		}
