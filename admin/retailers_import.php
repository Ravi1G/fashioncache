...
if (isset($_POST["action"]) && $_POST["action"] == "import")
{
	...
	if (count($errors) == 0)
	{
		...
		{
			header("Location: retailers_import.php?err=delimiter");
			exit();
		}*/
		...
		foreach ($csv->data as $key => $row)
		{
			...
			/*
			echo $title_e." - title<br/>";
			echo $network_e." - network<br/>";
			echo $program_e." - program<br/>";
			echo $category_e." - category<br/>";
			echo $image_e." - image url<br/>";
			echo $url_e." - url<br/>";
			echo $cashback_e." - cashback<br/>";
			echo $cashback_sign_e." - cashback sign<br/>";
			echo $description_e." - description<br/>";
			echo $conditions_e." - conditions<br/>";
			echo $featured_e." - featured<br/>";
			die();
			*/
			...
			if ($title_e && $network_e && $image_e && $url_e && $cashback_e && $cashback_sign_e)
			{
				...
				{
					$insert_sql = "INSERT INTO cashbackengine_retailers SET title='$title_e', network_id='$network_e', program_id='$program_e', url='$url_e', image='$image_e', cashback='".$cashback_e.$cashback_sign_e."', conditions='$conditions_e', description='$description_e', retailer_url='".friendlyURL($title_e)."', featured='$featured_e', deal_of_week='0', status='active', added=NOW()";
					$result = smart_mysql_query($insert_sql);
		...
		header("Location: retailers_import.php?msg=done");
		exit();
	}
	else
	{
...
?>
	...
	<?php } elseif (isset($_GET['msg']) && ($_GET['msg']) == "done") { ?>
			<div class="success_box">Retailers have been successfully added</div>
	<?php } ?>


		<form enctype="multipart/form-data" action="" method="post" name="form1">
        <table width="100%" style="background: #F7F7F7; padding: 10px;" align="center" cellpadding="3" cellspacing="0" border="0">
		<tr>
