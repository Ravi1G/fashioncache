...
\*******************************************************************/
	...
	if (isset($_POST['action']) && $_POST['action'] == "editcontent")
	{
		...
		if (count($errs) == 0)
		{
			if ($content_id > 7) $content_name = "name='page', "; else $content_name = "";

			$sql = "UPDATE cashbackengine_content SET language='$clanguage', ".$content_name." link_title='$link_title', title='$page_title', description='$page_text', page_location='$page_location', page_url='', meta_description='$meta_description', meta_keywords='$meta_keywords', status='$status', modified=NOW() WHERE content_id='$content_id' LIMIT 1";
