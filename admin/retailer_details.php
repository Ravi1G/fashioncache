...
\*******************************************************************/
	...
	if (isset($_GET['id']) && is_numeric($_GET['id']))
	{
		...
		$query = "SELECT *, DATE_FORMAT(added, '%e %b %Y %h:%i %p') AS date_added, DATE_FORMAT(end_date, '%d %b %Y %h:%i') AS retailer_end_date, UNIX_TIMESTAMP(end_date) - UNIX_TIMESTAMP() AS time_left FROM cashbackengine_retailers WHERE retailer_id='rid' LIMIT 1";
...
?>
	 ...
	 ?>
		...
		<?php if ($row['deal_of_week'] == 1) { ?><div style="float: right; margin-right: 10px;"><img src="images/icons/deal_of_week.png" align="absmiddle" /> <span style="color:#777;">Store of the Week</span></div><?php } ?>
        <table width="90%" cellpadding="2" cellspacing="5" border="0" align="center">
			...
			<tr>
				...
				<td width="70%" valign="top"><?php echo GetStoreCountries($row['retailer_id']); ?></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td nowrap="nowrap" width="70%" valign="top"><a href="<?php echo $row['image']; ?>" target="_blank"><?php echo $row['image']; ?></a></td>
			</tr>
			<?php } ?>
			<tr>
				<td width="30%" valign="middle" align="right" class="tb1">URL:</td>
				<td width="70%" valign="top"><input type="text" class="textbox" size="88" style="border: 1px solid #EEE; padding: 5px 2px; background: #F3FEFF;" value="<?php echo $row['url']; ?>" /></td>
			</tr>
			<?php if ($row['cashback'] != "") { ?>
			<tr>
