...
if (isset($_POST["action"]) && $_POST["action"] == "process_payment")
{
	...
	if (count($errors) == 0)
	{
		...
		if ($status == "paid" && mysql_num_rows($tresult) > 0)
		{
			...
			if (mysql_num_rows($uresult) > 0)
			{
				...
				if ($urow['ref_id'] > 0)
				{
					smart_mysql_query("UPDATE cashbackengine_transactions SET status='confirmed', process_date=NOW() WHERE user_id='".(int)$urow['ref_id']."' AND payment_type='Refer a Friend Bonus' AND status='pending'");

					// Confirm referral commission
					if (REFERRAL_COMMISSION > 0)
					{
						$commission_amount = CalculatePercentage($trow['amount'], REFERRAL_COMMISSION);
						smart_mysql_query("INSERT INTO cashbackengine_transactions SET user_id='".(int)$urow['ref_id']."', ref_id='".(int)$urow['user_id']."', amount='$commission_amount', payment_type='Referral Commission', status='confirmed', process_date=NOW()");
					}
				}
			}
		}
...
?>
     ...
     <h2>Process Payment</h2>
		...
		<?php } ?>
            ...
            <table bgcolor="#F9F9F9" style="border-radius: 7px;" width="70%" cellpadding="3" cellspacing="5" border="0" align="center">
              <tr>
                <td width="220" valign="middle" align="right" class="tb1">Payment ID:</td>
